...
function closeExistingPeer(){ try{ if(state.peer) state.peer.destroy(); }catch(e){} state.peer=null; state.conn=null; state.conns=[]; }
...
function hostSetup(){
  ...
  state.peer = new Peer(state.hostId, {debug: 1, config:{iceServers:[{urls:'stun:stun.l.google.com:19302'},{urls:'stun:global.stun.twilio.com:3478'}]}});
  state.peer.on('open', () => {
...
function joinGame(){
  ...
  state.peer = new Peer(null, {debug: 1, config:{iceServers:[{urls:'stun:stun.l.google.com:19302'},{urls:'stun:global.stun.twilio.com:3478'}]}});
...
function onPlayerMessage(msg){
  ...
  if(msg.type==='round'){ state.round=msg.round; state.roundIndex=msg.roundIndex; state.scores=msg.scores||state.scores; state.startedAt=Date.now(); renderPlayerRound(msg.round); }
